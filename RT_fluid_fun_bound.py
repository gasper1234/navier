...
# boundary: true false grid, se pozna le pri divergenci hitrosti (upam)
...
def correct_diverg(vx_grid, vy_grid, bound, n):
	N_y = len(vx_grid)
	N_x = len(vx_grid[0])
	...
	for reps in range(n):
		for y in range(N_y):
			for x in range(N_x):
				ind_ym, ind_yp = y-1, (y+1) % N_y
				ind_xm, ind_xp = x-1, (x+1) % N_x
				# preverimo če je katera stvar že v meji!!, najprej za x nato za y
				if bound[y, ind_xm] or bound[y, ind_xp]:
					if bound[ind_ym, x] or bound[ind_ym, x]:
						pass
					else:
						grad[y, x] = vy_grid[ind_yp, x] - vy_grid[ind_ym, x]
				else:
					if bound[ind_ym, x] or bound[ind_ym, x]:
						grad[y, x] = vx_grid[y, ind_xp] - vx_grid[y, ind_xm]
					else:
...
def clear_speed(vx_grid, vy_grid, bound):
	N_y = len(vx_grid)
	N_x = len(vx_grid[0])
	for y in range(N_y):
		for x in range(N_x):	
